"""Scene 1, English voice-over v2: Kokoro-82M (natural, Apache-2.0, free for
commercial use) instead of Piper (too robotic-sounding), plus the corrected
shot2 close-up crop (the old crop file was stale, from before the ControlNet
proportion fix, and had a glossy artifact over one eye).

Run with the TTS venv, not the ComfyUI venv:
    production/tts_venv/Scripts/python.exe make_scene1_video_en_v2.py
"""
import os
import subprocess
import numpy as np
...
def synthesize(pipeline, text, dest_wav):
...
def make_clip(shot):
    ...
    dest = os.path.join(OUT_DIR, f"{shot['name']}_panzoom_en2.mp4")
...
def concat_media(paths, dest, is_audio):
    list_path = os.path.join(OUT_DIR, "_concat_list_en2.txt")
...
if __name__ == "__main__":
    ...
    print("Synthesizing Kokoro voice-over segments...")
    for shot in SHOTS:
        wav_path = os.path.join(OUT_DIR, f"{shot['name']}_vo_en2.wav")
        vo_dur = synthesize(pipeline, shot["vo"], wav_path)
        shot["_vo_dur"] = vo_dur
    ...
    for shot in SHOTS:
        ...
        silence_path = os.path.join(OUT_DIR, f"{shot['name']}_pad_en2.wav")
        make_silence(silence_path, PAD_SECONDS)
        audio_parts.append(silence_path)
    audio_track = os.path.join(OUT_DIR, "s1_voiceover_en2.wav")
    concat_media(audio_parts, audio_track, is_audio=True)

    print("Concatenating video...")
    video_track = os.path.join(OUT_DIR, "episode01_scene1_video_only_en2.mp4")
    ...
    print("Muxing audio...")
    with_audio = os.path.join(OUT_DIR, "_episode01_scene1_en2_noSubs.mp4")
    cmd = [
    ...
    for shot in SHOTS:
        ...
        os.remove(os.path.join(OUT_DIR, f"{shot['name']}_pad_en2.wav"))
        os.remove(os.path.join(OUT_DIR, f"{shot['name']}_panzoom_en2.mp4"))
